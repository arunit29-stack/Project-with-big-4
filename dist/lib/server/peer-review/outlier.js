...
exports.calculateZScoresForSubmission = calculateZScoresForSubmission;
exports.getOutlierFlags = getOutlierFlags;
exports.getAssignmentOutlierFlags = getAssignmentOutlierFlags;
/**
...
function calculateStats(values) {
    ...
    const variance = values.reduce((a, val) => a + Math.pow(val - mean, 2), 0) / values.length;
 ...
 */
async function calculateZScoresForSubmission(assignmentId, submissionId) {
    ...
    try {
        ...
        const scoresRes = await client.query(`SELECT id, review_token, criterion, score, is_overridden, overridden_score
       ...
       ORDER BY criterion, review_token`, [submissionId]);
        ...
        for (const [criterion, criterionScores] of scoresByCriterion) {
            ...
            for (const score of criterionScores) {
                ...
                await client.query(`UPDATE peer_review_scores
           SET z_score = $1, is_outlier = $2, updated_at = NOW()
           WHERE id = $3`, [zScore, isOutlier, score.id]);
                if (isOutlier) {
        ...
        for (const detection of outlierDetections) {
            ...
            const existingRes = await client.query(`SELECT id FROM peer_review_outlier_flags
         WHERE assignment_id = $1 AND submission_id = $2 
           AND review_token = $3 AND criterion = $4
           AND resolved_at IS NULL`, [assignmentId, submissionId, detection.reviewToken, detection.criterion]);
            if (existingRes.rows.length === 0) {
                const flagId = (0, crypto_1.randomUUID)();
                await client.query(`INSERT INTO peer_review_outlier_flags (
            ...
            score, z_score, threshold
                ...
                // Audit log
                await client.query(`INSERT INTO peer_review_audit_log (
 ...
 */
async function getOutlierFlags(submissionId) {
    const pool = (0, postgres_1.getPostgresPool)();
    const res = await pool.query(`SELECT id, review_token, criterion, score, z_score, threshold, created_at, resolved_at, resolution_action
 ...
 */
async function getAssignmentOutlierFlags(assignmentId) {
    const pool = (0, postgres_1.getPostgresPool)();
    const res = await pool.query(`SELECT id, review_token, submission_id, criterion, score, z_score
     FROM peer_review_outlier_flags
