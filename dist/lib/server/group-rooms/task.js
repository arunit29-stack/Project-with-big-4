...
exports.getRoomKanban = getRoomKanban;
exports.getTaskAuditLog = getTaskAuditLog;
exports.getRoomTasks = getRoomTasks;
...
async function getTask(taskId) {
    const pool = (0, postgres_1.getPostgresPool)();
    const res = await pool.query(`SELECT id, room_id, title, description, assigned_to, created_by, status, due_date, created_at, updated_at
     FROM group_room_tasks WHERE id = $1`, [taskId]);
    if (res.rowCount === 0) {
        return null;
    }
 ...
 */
async function updateTask(taskId, changedBy, request) {
    const pool = (0, postgres_1.getPostgresPool)();
    // Get current task to compare status
    const currentTaskRes = await pool.query(`SELECT room_id, status FROM group_room_tasks WHERE id = $1`, [taskId]);
    ...
    try {
        ...
        if (newStatus !== oldStatus) {
            await pool.query(`INSERT INTO task_audit_log (id, task_id, room_id, changed_by, old_status, new_status)
         VALUES ($1, $2, $3, $4, $5, $6)`, [(0, crypto_1.randomUUID)(), taskId, roomId, changedBy, oldStatus, newStatus]);
            ...
            if (newStatus === "done") {
                ...
                await pool.query(`INSERT INTO contribution_metrics (id, room_id, student_id, metric_date, task_completions)
           VALUES ($1, $2, $3, $4, 1)
           ON CONFLICT (room_id, student_id, metric_date)
           DO UPDATE SET task_completions = task_completions + 1`, [(0, crypto_1.randomUUID)(), roomId, assignedTo, today]);
            }
        }
 ...
 */
async function getRoomKanban(roomId) {
    const pool = (0, postgres_1.getPostgresPool)();
    const res = await pool.query(`SELECT id, room_id, title, description, assigned_to, created_by, status, due_date, created_at, updated_at
     FROM group_room_tasks WHERE room_id = $1
     ORDER BY status, created_at ASC`, [roomId]);
 ...
 */
async function getTaskAuditLog(taskId) {
    const pool = (0, postgres_1.getPostgresPool)();
    const res = await pool.query(`SELECT id, task_id, room_id, changed_by, old_status, new_status, changed_at
