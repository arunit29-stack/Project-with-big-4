...
exports.voidQuestionAndRecalculateWithResponse = voidQuestionAndRecalculateWithResponse;
/* eslint-disable */
 ...
 * @returns Quiz ID
 */
async function createAiQuiz(courseId, title, aiQuestions) {
    const pool = (0, postgres_1.getPostgresPool)();
    const quizId = (0, crypto_1.randomUUID)();
    await pool.query("BEGIN");
    try {
        // Create quiz with draft status
        await pool.query(`INSERT INTO quizzes (id, course_id, title, status)
       VALUES ($1, $2, $3, 'draft')`, [quizId, courseId, title]);
 ...
 */
async function editQuestion(questionId, edit) {
    const pool = (0, postgres_1.getPostgresPool)();
    // Check if question is draft
    const qRes = await pool.query(`SELECT status FROM quiz_questions WHERE id = $1`, [questionId]);
    if (qRes.rowCount === 0) {
        throw new Error("question_not_found");
    }
...
async function deleteQuestion(questionId) {
    const pool = (0, postgres_1.getPostgresPool)();
    // Check if question is draft
    const qRes = await pool.query(`SELECT quiz_id, question_index, status FROM quiz_questions WHERE id = $1`, [questionId]);
    if (qRes.rowCount === 0) {
        throw new Error("question_not_found");
    }
    ...
    try {
        ...
        await pool.query(`UPDATE quiz_questions
       SET question_index = question_index - 1
       WHERE quiz_id = $1 AND question_index > $2`, [quizId, deletedIndex]);
        await pool.query("COMMIT");
    }
    catch (err) {
        await pool.query("ROLLBACK");
...
async function addQuestion(quizId, question) {
    ...
    const quizRes = await pool.query(`SELECT status FROM quizzes WHERE id = $1`, [quizId]);
    if (quizRes.rowCount === 0) {
    ...
    const pointValue = (_a = question.pointValue) !== null && _a !== void 0 ? _a : 10;
    if (pointValue < 1 || pointValue > 100) {
        throw new Error("pointValue must be between 1 and 100");
    }
    const timeLimitSeconds = (_b = question.timeLimitSeconds) !== null && _b !== void 0 ? _b : 30;
    ...
    const indexRes = await pool.query(`SELECT MAX(question_index) as max_index FROM quiz_questions WHERE quiz_id = $1`, [quizId]);
    const nextIndex = ((_d = (_c = indexRes.rows[0]) === null || _c === void 0 ? void 0 : _c.max_index) !== null && _d !== void 0 ? _d : -1) + 1;
    ...
    await pool.query(`INSERT INTO quiz_questions (
      ...
      point_value, time_limit_seconds, question_index, status, generated_by_ai
        ...
        (_e = question.correctOptionIndex) !== null && _e !== void 0 ? _e : null,
        (_f = question.explanation) !== null && _f !== void 0 ? _f : null,
        pointValue,
        timeLimitSeconds,
        nextIndex,
    ]);
...
async function publishQuiz(quizId) {
    ...
    try {
        ...
        const countRes = await pool.query(`SELECT COUNT(*) as count FROM quiz_questions WHERE quiz_id = $1 AND status = 'draft'`, [quizId]);
        ...
        await pool.query(`UPDATE quiz_questions SET status = 'published' WHERE quiz_id = $1 AND status = 'draft'`, [quizId]);
        // Update quiz status to published
        await pool.query(`UPDATE quizzes SET status = 'published' WHERE id = $1`, [quizId]);
        await pool.query("COMMIT");
 ...
 */
async function voidQuestionAndRecalculateWithResponse(quizId, questionId) {
    const pool = (0, postgres_1.getPostgresPool)();
    ...
    try {
        ...
        const attemptsRes = await pool.query(`SELECT id, user_id, score FROM quiz_attempts WHERE quiz_id = $1`, [quizId]);
        for (const attempt of attemptsRes.rows) {
            const attemptId = attempt.id;
            const userId = attempt.user_id;
            const oldScore = attempt.score;
            const scale = activeInitialPoints > 0 ? totalInitialPoints / activeInitialPoints : 0;
            const answersRes = await pool.query(`SELECT a.id, a.question_id, a.is_correct, a.time_remaining_seconds, a.points_awarded, q.point_value, q.time_limit_seconds
         FROM quiz_attempt_answers a
         JOIN quiz_questions q ON a.question_id = q.id
         WHERE a.attempt_id = $1`, [attemptId]);
            let totalScore = 0;
            ...
            // Update XP ledger
            await pool.query(`UPDATE student_xp_ledger SET xp_amount = $3 WHERE user_id = $1 AND quiz_id = $2`, [userId, quizId, Math.round(totalScore)]);
