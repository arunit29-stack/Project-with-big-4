...
async function apiRequest(url, method, body, token) {
    ...
    if (res.status === 409) {
        return Object.assign({ status: 409 }, (await res.json()));
    }
    return res.json();
}
async function runTests() {
    ...
    try {
        ...
        const quizRes = await apiRequest(`${baseUrl}/courses/course-bio-101/quizzes`, "POST", quizPayload, teacherToken);
        ...
        const pool = app.server; // fastify instance's pg connection is direct or we query DB
        const { getPostgresPool } = require("../lib/server/db/postgres");
        const dbPool = getPostgresPool();
        const questionsRes = await dbPool.query(`SELECT * FROM quiz_questions WHERE quiz_id = $1 ORDER BY question_index ASC`, [quizId]);
        const questions = questionsRes.rows;
        console.log(`Fetched ${questions.length} questions from DB.`);
        // 3. Launch lobby
        console.log("Launching quiz lobby...");
        const launchRes = await apiRequest(`${baseUrl}/quizzes/${quizId}/launch`, "POST", {}, teacherToken);
        console.log(`Lobby launched. Ends at: ${launchRes.lobbyEndsAt}`);
        // 4. Connect sockets
        console.log("Connecting student and teacher sockets...");
        const teacherSocket = (0, socket_io_client_1.io)(`${baseUrl}/quizzes/${quizId}`, {
            auth: { token: teacherToken },
            transports: ["websocket"],
        });
        const studentSocket = (0, socket_io_client_1.io)(`${baseUrl}/quizzes/${quizId}`, {
            auth: { token: studentToken },
            transports: ["websocket"],
        });
        let studentJoinedReceived = false;
        teacherSocket.on("quiz:student_joined", (data) => {
        ...
        const extendRes = await apiRequest(`${baseUrl}/quizzes/${quizId}/lobby/extend`, "POST", {}, teacherToken);
        console.log(`Lobby extended. New end time: ${extendRes.lobbyEndsAt}`);
        // 6. Start Quiz
        console.log("Starting quiz...");
        let questionPushed = false;
        let questionData = null;
        studentSocket.on("quiz:question", (data) => {
        ...
        const answerRes = await apiRequest(`${baseUrl}/quizzes/${quizId}/attempts/${attemptId}/answers`, "POST", {
        ...
        const stateRes = await apiRequest(`${baseUrl}/quizzes/${quizId}/attempts/${attemptId}/state`, "GET", null, studentToken);
        ...
        const duplicateRes = await apiRequest(`${baseUrl}/quizzes/${quizId}/attempts/attempt-uuid-2222/answers`, "POST", {
            questionId: questions[0].id,
            selectedOption: "1",
        ...
        const logsRes = await dbPool.query(`SELECT * FROM quiz_integrity_log WHERE quiz_id = $1 AND user_id = $2`, [quizId, studentUserId]);
        ...
        const answer2Res = await apiRequest(`${baseUrl}/quizzes/${quizId}/attempts/${attemptId}/answers`, "POST", {
        ...
        const attemptRow = await dbPool.query(`SELECT * FROM quiz_attempts WHERE id = $1`, [attemptId]);
        ...
        const ledgerRow = await dbPool.query(`SELECT * FROM student_xp_ledger WHERE quiz_id = $1 AND user_id = $2`, [quizId, studentUserId]);
        ...
        const voidRes = await apiRequest(`${baseUrl}/quizzes/${quizId}/questions/${questions[0].id}/void`, "POST", {}, teacherToken);
        ...
        // Let's verify this.
        const attemptVoidRow = await dbPool.query(`SELECT * FROM quiz_attempts WHERE id = $1`, [attemptId]);
