...
function generateAnonymousId(): string {
  return `REDACTED_${randomUUID().replace(/-/g, "").substring(0, 12)}`;
}
...
export async function purgeUserPII(
  ...
  try {
    ...
    await client.query(
      `UPDATE users SET email = $1, name = $2, anonymised_at = NOW() WHERE id = $3`,
      [anonId + "@redacted.local", anonId, userId]
    );
    ...
    const dmRes = await client.query(
      `UPDATE direct_message_content SET body = '[Content removed]' WHERE sender_id = $1 OR recipient_id = $1`,
      [userId]
    );
    ...
    const chatRes = await client.query(
      `UPDATE room_chat_messages SET text = '[Content removed]' WHERE sender_id = $1`,
      [userId]
    );
    ...
    const reviewTokenRes = await client.query(
      `DELETE FROM review_tokens WHERE reviewer_id = $1`,
      [userId]
    );
    ...
    const peerReviewRes = await client.query(
      `UPDATE peer_review_assignments SET submitter_id = NULL WHERE submitter_id = $1`,
      [userId]
    );
    ...
    const submissionsRes = await client.query(
      `UPDATE submissions SET file_key = NULL, file_name = NULL, content = '[Content removed]' 
       WHERE user_id = $1`,
      [userId]
    );
    ...
    const contributionRes = await client.query(
      `DELETE FROM contribution_metrics WHERE student_id = $1`,
      [userId]
    );
    ...
    const annotationsRes = await client.query(
      `UPDATE pdf_annotations SET user_id = NULL, content = '[Content removed]' WHERE user_id = $1`,
      [userId]
    );
    ...
    const aiLogsRes = await client.query(
      `UPDATE ai_query_logs SET query = '[Content removed]', response = '[Content removed]' 
       WHERE user_id = $1`,
      [userId]
    );
    ...
    const quizAttemptsRes = await client.query(
      `UPDATE quiz_attempts SET anonymised_at = NOW() WHERE user_id = $1`,
      [userId]
    );
    ...
    const videoNotesRes = await client.query(
      `DELETE FROM video_notes WHERE user_id = $1`,
      [userId]
    );
    ...
    const inactivityRes = await client.query(
      `DELETE FROM inactivity_reports WHERE reporter_id = $1 OR reported_student_id = $1`,
      [userId]
    );
    ...
    await client.query(
      `INSERT INTO gdpr_purge_audit_log (
 ...
 */
export async function getGDPRPurgeLog(purgeId: string): Promise<any> {
  const pool = getPostgresPool();

  const res = await pool.query(
    `SELECT * FROM gdpr_purge_audit_log WHERE id = $1`,
    [purgeId]
  );
 ...
 */
export async function listGDPRPurges(institutionId: string): Promise<any[]> {
  const pool = getPostgresPool();

  const res = await pool.query(
    `SELECT id, admin_id, user_id, records_anonymised, files_deleted, created_at 
