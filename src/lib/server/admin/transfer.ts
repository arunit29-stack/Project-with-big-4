...
export async function transferCourse(
...
): Promise<CourseTransferResponse> {
  ...
  try {
    ...
    const newTeacherRes = await client.query(
    ...
    await client.query(
      `UPDATE courses SET created_by = $1, updated_at = NOW() WHERE id = $2`,
      [newTeacherId, courseId]
    );

    // Transfer assignments and their submissions
    const assignmentsRes = await client.query(
      `UPDATE assignments SET created_by = $1 WHERE course_id = $2
       RETURNING id`,
      [newTeacherId, courseId]
    );
    ...
    const quizzesRes = await client.query(
      `UPDATE quizzes SET created_by = $1 WHERE course_id = $2
       RETURNING id`,
      [newTeacherId, courseId]
    );
    ...
    const groupRoomsRes = await client.query(
      `UPDATE group_rooms SET created_by = $1 WHERE course_id = $2
       RETURNING id`,
      [newTeacherId, courseId]
    );
    ...
    const videoRes = await client.query(
      `UPDATE video_library SET created_by = $1 WHERE course_id = $2
       RETURNING id`,
      [newTeacherId, courseId]
    );
    ...
    const dmRes = await client.query(
      `UPDATE direct_messages SET created_by = $1 WHERE course_id = $2
       RETURNING id`,
      [newTeacherId, courseId]
    );
    ...
    // Create audit log
    await client.query(
      `INSERT INTO course_transfer_audit (
